...
function property_submission_form_shortcode() {
    ...
    // Localize script with needed variables - MOVED HERE FROM BOTTOM OF FUNCTION
   ...
   echo '<style>
    ...
    max-width: 1200px;
    margin: 0 auto;
	font-family: "lato", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Oxygen-Sans, Ubuntu, Cantarell, "Helvetica Neue", Arial, sans-serif;
    color: #333;
...
@media (max-width: 768px) {
    ...
    // Tag Input System - FIXED IMPLEMENTATION
    function initTagSystem(inputId, containerId, suggestionsId, taxonomy) {
        ...
        $('#' + inputId).on('keyup focus', function() {
            ...
            // AJAX request to search for tags - FIXED AJAX URL AND NONCE PARAMETER
            $.ajax({
                url: property_submission_vars.ajaxurl,
                data: {
                ...
                error: function(xhr, status, error) {
                    console.log('Error searching for taxonomy terms:', error);
                }
            });
        });
        ...
        $(document).on('click', '#' + suggestionsId + ' .tag-suggestion-item', function() {
            ...
            if ($(this).hasClass('new-tag')) {
                // AJAX request to create a new term - FIXED AJAX URL AND NONCE PARAMETER
                $.ajax({
                    url: property_submission_vars.ajaxurl,
                    method: 'POST',
                    data: {
                    ...
                    error: function(xhr, status, error) {
                        console.log('Error creating new taxonomy term:', error);
                    }
                });
            } else {
                selectedTags.push({
    ...
    // AJAX handlers for taxonomy terms - FIXED BY REMOVING DUPLICATES
    return ob_get_clean();
}

// Define AJAX handlers outside of the shortcode function
...
function enqueue_property_submission_scripts() {
    ...
    // Then localize it with the required variables
    wp_localize_script(
        'property-submission-script',
        'property_submission_vars',
        array(
            'ajaxurl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('property_submission_nonce')
        )
...
add_action('wp_enqueue_scripts', 'enqueue_property_submission_scripts');
...
/**
 * Enqueue scripts and localize variables for property submission
 */
function enqueue_property_submission_scripts() {
    // Enqueue your main script
    wp_enqueue_script(
        'property-submission-script', 
        get_template_directory_uri() . '/assets/js/property-submission.js', 
        array('jquery'), 
        '1.0.1', 
        true
    );
    
    // Get existing taxonomy terms for the user if needed
    $existing_terms = array();
    
    // Example: If editing a property, get its terms
    if (isset($_GET['edit']) && !empty($_GET['edit'])) {
        $property_id = intval($_GET['edit']);
        
        // Get property amenities
        $amenities = get_the_terms($property_id, 'property_amenity');
        if (!empty($amenities) && !is_wp_error($amenities)) {
            $existing_terms['property_amenity'] = array_map(function($term) {
                return array(
                    'id' => $term->term_id,
                    'name' => $term->name
                );
        ...
        if (!empty($features) && !is_wp_error($features)) {
            $existing_terms['property_feature'] = array_map(function($term) {
                return array(
                    'id' => $term->term_id,
                    'name' => $term->name
                );
    ...
    // Localize the script with required variables
    wp_localize_script(
        'property-submission-script',
        'property_submission_vars',
        array(
            'ajaxurl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('property_submission_nonce'),
            'property_amenity' => isset($existing_terms['property_amenity']) ? $existing_terms['property_amenity'] : array(),
            'property_feature' => isset($existing_terms['property_feature']) ? $existing_terms['property_feature'] : array(),
            // Add other taxonomies as needed
        )
    );
}
...
function search_taxonomy_terms_callback() {
    ...
    if (!isset($_REQUEST['nonce']) || !wp_verify_nonce($_REQUEST['nonce'], 'property_submission_nonce')) {
        wp_send_json_error('Security check failed');
    }
...
add_action('wp_ajax_nopriv_search_taxonomy_terms', 'search_taxonomy_terms_callback'); // If needed for non-logged in users

/**
 * AJAX handler for creating new taxonomy terms
 */
function create_taxonomy_term_callback() {
    // Check nonce for security
    if (!isset($_REQUEST['nonce']) || !wp_verify_nonce($_REQUEST['nonce'], 'property_submission_nonce')) {
        wp_send_json_error('Security check failed');
    }
    
    // Only allow logged-in users to create terms
    if (!is_user_logged_in()) {
        wp_send_json_error('You must be logged in to create new terms');
    }
    ...
    if ($existing_term) {
        wp_send_json_success(array('term_id' => $existing_term['term_id']));
    }
    ...
    wp_send_json_success(array('term_id' => $result['term_id']));
}
add_action('wp_ajax_create_taxonomy_term', 'create_taxonomy_term_callback');
// No need for nopriv version as non-logged in users shouldn't create terms